...
async function fetchPopularMovies() {
    try {
        ...
        let response = await fetch(`${apiUrl}/movie/popular?api_key=${apiKey}&language=en-US&page=1`);
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            ...
            const response = await fetch(`${apiUrl}/search/movie?api_key=${apiKey}&language=en-US&query=${encodeURIComponent(query)}`);
            const data = await response.json();
            displayMovies(data.results);
